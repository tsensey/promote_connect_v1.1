...
function normalizeKey(key: string): string {
  ...
  if (/mail1|email1|mail|email/.test(lowered) && !lowered.includes('2')) return 'mail1';
...
function buildInsert(row: FlatExposant, pavillonToEspace: Map<string, { id: string; code: string }>): ExposantInsert | null {
  ...
  let email_contact = row.email_contact?.trim() || null;
  if (row.mail2?.trim()) {
    email_contact = email_contact
      ? `${email_contact}, ${row.mail2.trim()}`
      : row.mail2.trim();
  }

  let phone_contact = row.phone_contact?.trim() || null;
  if (row.tel2?.trim()) {
    phone_contact = phone_contact
      ? `${phone_contact}, ${row.tel2.trim()}`
      : row.tel2.trim();
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!sheetName) {
      return NextResponse.json({ error: 'Aucune feuille trouvee dans le fichier.' }, { status: 400 });
    }

    const jsonData = XLSX.utils.sheet_to_json<Record<string, unknown>>(workbook.Sheets[sheetName], { defval: '' });
    if (!jsonData || jsonData.length === 0) {
      return NextResponse.json({ error: 'Aucune donnee trouvee dans le fichier.' }, { status: 400 });
    }
    ...
    records = jsonData.map((row) => {
      const r: FlatExposant = {
        ...
        email_contact: null, phone_contact: null,
        logo_url: null, reseaux: null, adresse: null,
        mail2: null, site2: null, tel2: null,
      };
      for (const [originalKey, normalizedKey] of keyMap) {
        ...
        else if (normalizedKey === 'mail1') r.email_contact = val;
    ...
    return NextResponse.json({ error: 'Erreur de lecture du fichier. Verifiez le format.' }, { status: 400 });
  }
  ...
  if (inserts.length === 0) {
    return NextResponse.json({
      error: 'Aucun exposant valide. Verifiez que la colonne "Raison Sociale" contient des valeurs.',
    }, { status: 400 });
  }

  const { error } = await supabase.from('exposants').insert(inserts);
  if (error) {
