-- Exécution forcée de la création des vues pour n8n (suite à l'échec transactionnel de la migration 049)
...
-- 4. Vue pour les relances d'abonnements expirant dans 7 jours (avec le nouveau nom pour éviter les conflits)
create or replace view v_n8n_expiring_subscriptions as
select 
  p.id, 
  p.full_name, 
...
revoke all on v_n8n_expiring_subscriptions from public, authenticated, anon;
grant select on v_n8n_expiring_subscriptions to service_role;
